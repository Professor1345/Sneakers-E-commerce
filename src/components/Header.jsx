...
const Header = ({ inputNumberCart, setInputNumberCart }) => {
        ...
        >
          ...
          <div
            className={`notification ${
              inputNumberCart == undefined ? "hidden" : "block"
            } ${
              inputNumberCart >= 100
                ? "left-2 text-[10px]"
                : "left-3 sm:left-2 text-xs"
            } ${isCart ? "invert" : "invert-0"}`}
          >
            {Number(inputNumberCart) >= 100 ? "99+" : inputNumberCart}
          </div>
        </div>
        <div className={`cart ${isCart ? "block" : "hidden"}`}>
          <div className="py-4 px-5 font-bold text-[hsl(220,_13%,_13%)]">
            Cart
          </div>
          <hr />
          {inputNumberCart != undefined ? (
            <div className="cart-content">
              <div className="flex flex-row items-center justify-between text-[15px] 2xs:text-base sm:text-sm md:text-base">
                <div className="overflow-hidden h-[3rem] w-[2.8rem]">
                  <img src={product01} alt="product" className="size-full" />
                </div>
                <div className="mx-2">
            ...
            <div className="empty">
              <div className="text-[hsl(219,9%,45%)] font-bold">
                Your cart is empty.
              </div>
            </div>
          )}
